...
class labCollabLM(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.saveScores(y_score, y_true)
        return
    ...
    def saveScores(self, class_score, class_true):
        class_score = class_score.cpu().detach().numpy().flatten()
        class_true = class_true.cpu().detach().numpy().flatten()

        savePath = 'outputs/tempScores/epoch.csv'.replace('epoch', str(self.current_epoch))

        scoreDict = {'class_score': class_score, 'class_true': class_true}
